...
<a href="assets/images/posters/sculpture-classes.jpg"><img src="assets/images/posters/sculpture-classes.jpg" class="rimg" width="120"/></a> The mm-ADT virtual machine coordinates processing engines and storage systems within a distributed, multi-machine compute cluster. The machine's processor abstraction integrates with the reactive pattern, the actor model, and/or various asynchronous/batch message-passing architectures. Processors implement a core set of primitive functional operations that are composed into semi-cyclic _instruction graphs_ called programs (or queries). Program instruction graphs guide swarms of traverser-monads across cluster data categorized by a traverser-computable type system whose programs are compiled by traversers using abstract interpretation. mm-ADT type definitions are typically constrained to instruction subsets that delineate computational boundaries for canonical-types, refinement-types, dependent-types, and ultimately "Turing types" specified with looping and branching instruction combinators. Universal computing is achieved over storage systems ranging from databases w/ complicated data access paths, to distributed file-systems optimized for linear-access batch processors. The mm-ADT algebras are codified in traverser-accessible _models_ that define functors to translate traversers across ADTs&mdash;thus, enabling the inter-communication of numerous disparate data technologies.
