...
<a href="assets/images/posters/sculpture-classes.jpg"><img src="assets/images/posters/sculpture-classes.jpg" class="rimg" width="120"/></a> The mm-ADT virtual machine coordinates processing engines and storage systems within a distributed, multi-machine cluster. The machine's processor abstraction evaluates in frameworks based on the reactive pattern, the actor model, and/or the many asynchronous/batch message-passing architectures. Processors implement a core set of functional operators that can be composed into semi-cyclic _instruction graphs_ called programs (or queries). Program instruction graphs guide swarms of traverser-monads across the cluster's data using an computable type system optimized by traversers using abstract interpretation aimed at the instruction graph ADT. mm-ADT types are themselves programs typically constrained to instruction subsets delineating canonical-types, refinement-types, and dependent-types. Ultimately "Turing types" can be  constructed with looping and branching instruction combinators. Universal computing is achieved over storage systems ranging from databases w/ complicated data access paths, to distributed file-systems optimized for linear-access batch processors. The mm-ADT system algebras driving this integration is codified in traverser-accessible mm-ADT _models_ that specify type/instruction functors that translate traversers and their state across ADTs&mdash;thus, enabling the inter-communication of numerous disparate data technologies.
